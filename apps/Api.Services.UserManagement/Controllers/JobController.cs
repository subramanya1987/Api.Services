...
namespace Api.Services.UserManagement.Controllers
{
    ...
    [ApiController]
    public class JobController : Api.Services.Infra.BaseController
    {
        ...
        public async Task<IActionResult> GetUserList()
        {
            ...
            var identifierId = $"JobController.GetUserList {Guid.NewGuid().ToString()}";
            using (LogContext.PushProperty("CorrelationId", identifierId))
            {
                _logger.LogInformation("GetUserList called with identifier {Identifier}", identifierId);
